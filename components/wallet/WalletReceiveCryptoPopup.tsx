...
export default function WalletReceiveCryptoPopup() {
  ...
  const handleSaveQR = async () => {
    ...
    try {
      ...
      link.download = `swop-qr-${privyUser?.email?.address?.split("@")[0] || "user"}.png`;
          ...
          <div className="flex flex-col items-center mb-6">
            ...
            >
              ...
              <div className="text-center">
                <p className="text-sm font-medium text-gray-900">Powered By Swop</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 shadow-medium rounded-xl p-4">
            ...
            <div className="text-start space-y-3 text-gray-500">
              <p className="text-sm leading-relaxed">
                Your swop.id is your universal identity for messaging, payments,
                and publishing.
              </p>
              <p className="text-sm leading-relaxed">It gives you:</p>
              <div className="text-left text-sm space-y-1 px-2">
                <p>• One link to receive encrypted messages, post updates, and interact with bots.</p>
                <p>• One address to receive stablecoin payments instantly.</p>
                <p>• One profile where your content, feed, and assets live — always on-chain, always yours.</p>
              </div>
              <p className="text-sm leading-relaxed">
                Share your swop.id to stay connected, get paid, and grow your presence in one place.
              </p>
            </div>

            <div className="text-center">
              <PrimaryButton onClick={handleShareUsername} className="w-full py-2">
                Share your username
              </PrimaryButton>
            </div>
          </div>
            ...
            {chainAddresses.map((chain, index) => (
              ...
              >
                ...
                <div className="w-10 h-10 rounded-full flex items-center justify-center">
                  <Image src={chain.icon} alt={chain.name} width={120} height={120} />
                </div>

                <p className="text-xs text-gray-600 font-mono">
                  {chain.address
                    ? `${chain.address.slice(0, 4)}...${chain.address.slice(-4)}`
                    : ""}
                </p>
