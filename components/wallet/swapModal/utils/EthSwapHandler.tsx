...
const throttleRequests = <T extends (...args: any[]) => Promise<any>>(
  fn: T,
  delay: number
): T => {
  let lastCall = 0;
...
export default function EthSwapHandler({
  ...
  const [sepoliaProvider, setSepoliaProvider] =
    useState<ethers.Provider | null>(null);

  // Get the appropriate network configuration based on active network
  const networkConfig =
    activeNetwork === 'sepolia' ? NETWORKS.SEPOLIA : NETWORKS.MAINNET;
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      console.log(
        '[DEBUG] EthSwapHandler unmounting - setting mounted flag to false'
      );
  ...
  useEffect(() => {
    ...
    try {
      const mainnetRpcProvider = new ethers.JsonRpcProvider(
        RPC_URLS.mainnet
      );
      const sepoliaRpcProvider = new ethers.JsonRpcProvider(
        RPC_URLS.sepolia
      );
    ...
    const initWallet = async () => {
      try {
        ...
        try {
          ...
          console.error(
            'Failed to get provider/signer from Privy wallet:',
            err
          );
  ...
  const fetchTokenBalancesStable = useCallback(async () => {
    ...
    try {
      ...
      const mergedTokens = tokenList.map((token) => {
        const updatedToken = updatedTokensArray.find(
          (t) => t.symbol === token.symbol
        );
        if (updatedToken) return updatedToken;

        const existingToken = currentUserTokens.find(
          (t) => t.symbol === token.symbol
        );
  ...
  useEffect(() => {
    ...
    const newInputToken = getEthTokenInfoBySymbol(
      inputSymbol,
      userTokens,
      tokenList
    );
    const newOutputToken = getEthTokenInfoBySymbol(
      outputSymbol,
      userTokens,
      tokenList
    );
  ...
  useEffect(() => {
    console.log(
      `[DEBUG] Quote state changed:`,
      quote ? 'Has Quote' : 'No Quote'
    );
  }, [quote]);
  ...
  useEffect(() => {
    ...
    const fetchQuoteWithStateManagement = async () => {
      ...
      if (
        ...
        console.log(
          `[DEBUG] Skipping quote fetch - conditions not met`
        );
        return;
      }
      ...
      if (
        ...
        console.log(
          `[DEBUG] Skipping duplicate quote fetch - same parameters`
        );
        return;
      }
      ...
      try {
        // Log the request
        console.log(
          `Starting quote fetch for ${inputSymbol} to ${outputSymbol} on ${activeNetwork}`
        );
        ...
        // Get token addresses
        const inputTokenAddress = getTokenAddressBySymbol(
          inputSymbol,
          networkConfig
        );
        const outputTokenAddress = getTokenAddressBySymbol(
          outputSymbol,
          networkConfig
        );
        ...
        if (!inputTokenAddress || !outputTokenAddress) {
          console.warn(
            'Invalid token selection for network',
            activeNetwork
          );
          throw new Error(
            `Token not available on ${activeNetwork} network`
          );
        }
        ...
        console.log(
          `Fetching quote for ${inputSymbol} (${inputTokenAddress}) to ${outputSymbol} (${outputTokenAddress}) on ${activeNetwork}`
        );
        ...
        if (mountedRef.current) {
          if (newQuote) {
            ...
            console.log(
              '[DEBUG] Quote fetch successful, updating state'
            );
          ...
          console.log(
            '[DEBUG] Component unmounted before quote update could be applied'
          );
        }
      } catch (err: any) {
        // Type err as any
        ...
        if (mountedRef.current) {
          ...
          console.log(
            `[DEBUG] Component unmounted - not updating state`
          );
        }
        isLoadingQuoteRef.current = false;
      }
  ...
  useEffect(() => {
    console.log(
      `[DEBUG] Network change effect triggered: ${activeNetwork}`
    );
    ...
    if (provider && ethAddress) {
      setTimeout(() => {
        if (mountedRef.current) {
          console.log(
            `[DEBUG] Fetching token balances after network change`
          );
          fetchTokenBalancesStable();
        }
      }, 500);
  ...
  useEffect(() => {
    if (open && !wasOpen.current) {
      ...
      console.log(
        '[DEBUG] Modal opened - forcefully resetting all states'
      );
      resetAllState();
      ...
      setTimeout(() => {
        if (mountedRef.current) {
          console.log(
            '[DEBUG] Post-open delay completed - now ready to fetch quotes'
          );
        }
      }, 500);
    }
  ...
  const toggleNetwork = async () => {
    console.log(
      `[DEBUG] toggleNetwork called - current network: ${activeNetwork}`
    );
    ...
    // Toggle the network
    const newNetwork =
      activeNetwork === 'mainnet' ? 'sepolia' : 'mainnet';
    console.log(
      `[DEBUG] Setting activeNetwork from ${activeNetwork} to ${newNetwork}`
    );
    ...
    if (wallet) {
      try {
        ...
        console.log(
          `[DEBUG] Successfully requested chain switch to ${newNetwork}`
        );
        ...
        if (switchError.code === 4902) {
          try {
            ...
            console.log(
              `[DEBUG] Successfully added chain ${newNetwork}`
            );
          } catch (addError) {
            console.error(
              `[ERROR] Failed to add ${newNetwork} chain:`,
              addError
            );
          ...
          console.error(
            `[ERROR] Failed to switch to ${newNetwork}:`,
            switchError
          );
  ...
  const isWalletNetworkMismatch =
    ...
    ((activeNetwork === 'mainnet' &&
      walletChainId !== NETWORKS.MAINNET.chainId) ||
      (activeNetwork === 'sepolia' &&
        walletChainId !== NETWORKS.SEPOLIA.chainId));
  ...
  useEffect(() => {
    ...
    console.log('[DEBUG] Button state changed:', {
      ...
      buttonText: swapLoading
        ...
        : isWalletNetworkMismatch
        ? `Switch to ${
            activeNetwork === 'mainnet' ? 'Mainnet' : 'Sepolia'
          }`
        : !quote
        ? 'Get Quote'
      ...
      {isWalletNetworkMismatch && (
        <div className="p-3 mb-4 bg-yellow-100 text-yellow-700 rounded-lg text-sm">
          ...
          <p>
            ...
            {walletNetwork || 'another network'}. Switch to{' '}
            {activeNetwork === 'mainnet'
              ? 'Ethereum Mainnet'
              : 'Sepolia Testnet'}{' '}
            to execute swaps.
