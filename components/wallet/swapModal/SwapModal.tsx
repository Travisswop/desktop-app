...
export default function SwapModal({
  ...
  useEffect(() => {
    const fetchQuote = async () => {
      ...
      try {
        const decimals = swapMode === "SOL" ? 9 : 6; // Adjust based on token decimals
        const amountInSmallestUnit = Math.floor(
          parseFloat(amount) * 10 ** decimals
        );

        const res = await fetch(
          `https://quote-api.jup.ag/v6/quote?inputMint=${inputMint.toString()}&outputMint=${outputMint.toString()}&amount=${amountInSmallestUnit}&slippageBps=50&restrictIntermediateTokens=true`
        );
