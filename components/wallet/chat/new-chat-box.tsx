...
import { Loader, Send, MoreHorizontal, Reply, Edit3, Trash2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { usePrivy } from "@privy-io/react-auth";
import { useNewSocketChat, ChatMessage } from "@/lib/context/NewSocketChatContext";
...
const NewChatBox: React.FC<NewChatBoxProps> = ({ conversationId, receiverId }) => {
  const { user } = usePrivy();
  const {
    messages,
    sendMessage,
  ...
  console.log(`[NewChatBox] Messages count: ${conversationMessages.length}`);
  ...
  useEffect(() => {
    const loadConversation = async () => {
      ...
      setIsLoading(true);
      try {
        console.log(`[NewChatBox] Loading conversation with ${receiverId}`);
        
        // First join the conversation (like in HTML test)
        const joinResult = await joinConversation(receiverId);
        if (joinResult.success) {
          console.log(`[NewChatBox] Successfully joined conversation`);
        } else {
          console.error(`[NewChatBox] Failed to join conversation: ${joinResult.error}`);
        }
        
        // Then load conversation history
        const result = await getConversation(receiverId, 1, 50);
        if (result.success) {
          console.log(`[NewChatBox] Loaded ${result.data.messages?.length || 0} messages`);
          
          // Mark messages as read
          await markMessagesAsRead(receiverId);
        } else {
          console.error(`[NewChatBox] Failed to load conversation: ${result.error}`);
        }
      } catch (error) {
        console.error('[NewChatBox] Error loading conversation:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  }, [receiverId, user?.id, isConnected, joinConversation, getConversation, markMessagesAsRead]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const success = await sendMessage(receiverId, messageContent, 'text');
                    ...
                    <DropdownMenuTrigger asChild>
                      <Button
                        ...
                        className={`absolute -right-2 -top-2 h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity ${
                          isOwnMessage ? 'text-white hover:bg-white/20' : 'text-gray-500 hover:bg-gray-200'
                        }`}
                      >
