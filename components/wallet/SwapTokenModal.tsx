...
export default function SwapTokenModal({ tokens }: { tokens: any[] }) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          ...
          errorMessage =
            errorData?.error || "Unable to get price quote. Please try again.";
        }
  ...
  const getQuoteInfo = () => {
    ...
    if (jupiterQuote) {
      ...
      fromAmountUSD = quote.estimate?.fromAmountUSD || quote.fromAmountUSD;
    ...
    <div className="flex justify-center mt-10 relative">
      <Card className="w-full max-w-md p-4 rounded-2xl shadow-lg bg-white text-black">
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-center flex-1">Swap</h2>
          <button
...
//   // Check if this is a Solana-to-Solana swap
//   const isSolanaToSolanaSwap = () => {
//     return (
//       payToken?.chain === "SOLANA" &&
//       (receiveToken?.chain === "SOLANA" ||
...
//       // Get token addresses for Solana
//       const getTokenMint = (token: any) => {
//         if (token.symbol === "SOL") {
//           return "So11111111111111111111111111111111111111112";
//         }
...
//       const platfromFeeBps = 50; // need to work on
...
//         console.error("Jupiter API Error:", errorData);
//         throw new Error(
//           errorData?.error || `Jupiter API Error: ${response.status}`
//         );
//       }
...
//       // Check if this is Solana-to-Solana swap
//       if (isSolanaToSolanaSwap()) {
//         await executeJupiterSwap();
//       } else {
//         // Use Li.Fi for cross-chain swaps
//         await executeLiFiSwap();
//       }
//     } catch (error: any) {
//       console.error("Swap error:", error);
...
//   // Execute Jupiter swap for Solana-to-Solana
//   const executeJupiterSwap = async () => {
//     try {
//       if (!jupiterQuote) {
//         setSwapError("No Jupiter quote available");
...
//     } catch (error: any) {
//       console.error("Jupiter swap failed:", error);
//       setSwapError(error.message || "Jupiter swap failed");
//     } finally {
//       setIsSwapping(false);
//     }
//   };

//   // Execute Li.Fi swap for cross-chain
//   const executeLiFiSwap = async () => {
//     try {
//       if (!quote) {
//         setSwapError("No Li.Fi quote available");
...
//       if (fromChainId === 1151111081099710) {
//         // Solana transaction using Li.Fi
//         await executeSolanaSwap();
//       } else {
//         // EVM chains using Li.Fi
//         const allAccounts = PrivyUser?.linkedAccounts || [];
//         const ethereumAccount = allAccounts.find(
//           (account: any) =>
//             account.chainType === "ethereum" &&
...
//   // FIXED: getLifiQuote function with proper Solana handling and API key
//   // FIXED: getLifiQuote function with proper Solana handling and API key
//   const getLifiQuote = async () => {
//     try {
//       const queryParams = new URLSearchParams();
...
//       queryParams.append("toAddress", toWalletAddress);
//       queryParams.append("fromAmount", fromAmount);
//       queryParams.append("slippage", (slippage / 100).toString()); // Convert percentage to decimal

//       // Add integrator and fee for LiFi (0.5% fee)
//       queryParams.append("integrator", "swop");
//       queryParams.append("fee", "0.005");
...
//   // Quote fetching useEffect - handles both Jupiter and Li.Fi
//   useEffect(() => {
...
//         if (isSolanaToSolanaSwap()) {
//           // Use Jupiter for Solana-to-Solana swaps
//           console.log("Using Jupiter for Solana-to-Solana swap");
//           const jupiterQuote = await getJupiterQuote();

//           if (!isCancelled) {
//             setJupiterQuote(jupiterQuote);
//             setQuote(null); // Clear Li.Fi quote
//             console.log("Jupiter quote received:", jupiterQuote);
//           }
//         } else {
//           // Use Li.Fi for cross-chain swaps
...
//           if (!isCancelled) {
//             setQuote(lifiQuote);
//             setJupiterQuote(null); // Clear Jupiter quote
//             console.log("Li.Fi quote received:", lifiQuote);
//           }
//         }
//       } catch (error: any) {
...
//       setIsCalculating(false);
//     }
//   }, [quote, jupiterQuote, receiveToken, payAmount, payToken]);

//   // Solana swap execution using Li.Fi
//   const executeSolanaSwap = async () => {
//     try {
//       if (!solWallets || solWallets.length === 0) {
//         setSwapError("No Solana wallet connected");
...
//     if (jupiterQuote) {
//       // Jupiter doesn't provide USD values directly
//       fromAmountUSD = null;
//       toAmountUSD = null;
//       fees = jupiterQuote.platformFee || null;
//       priceImpact = jupiterQuote.priceImpactPct || null;
...
//             {/* Second row of slippage options */}
//             <div className="grid grid-cols-4 gap-2 mb-3">
//               <button
//                 onClick={() => handleSlippageChange(3.0)}
//                 className={`py-2 px-2 text-sm rounded-lg transition-all ${
...
//                       <span className="text-gray-900">
//                         {quoteInfo?.fees
//                           ? Array.isArray(quoteInfo.fees)
//                             ? quoteInfo.fees.length > 0
//                               ? `${quoteInfo.fees
//                                   .reduce((total, fee) => {
//                                     return (
