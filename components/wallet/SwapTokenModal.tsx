...
export default function SwapTokenModal({ tokens }: { tokens: any[] }) {
  ...
  const executeJupiterSwap = async () => {
    try {
      ...
      if (needsInputATA || needsOutputATA) {
        console.log("⚠️ [SWAP] Missing ATAs detected, creating them...");
        ...
        if (needsOutputATA) {
          console.log("➕ [SWAP] Adding output ATA creation instruction", {
        ...
        try {
          ...
          console.log("⏳ [SWAP] Confirming ATA creation:", signature);
      ...
      if (isEligibleForSponsorship && doSponsorship) {
        ...
        try {
          ...
          throw new Error(
            `Authorization signature failed: ${signError.message || signError}`
          );
        }
        ...
        const responseData = await sponsorResponse.json().catch(() => ({}));
        ...
        if (!sponsorResponse.ok) {
          ...
          throw new Error(
            responseData.error ||
              `Sponsored transaction failed (${sponsorResponse.status}: ${
        ...
        console.log("✅ [SWAP] Sponsored transaction submitted:", txId);
      } else {
      ...
      if (confirmationRpcUrl) {
        ...
        console.warn("⚠️ [SWAP] No RPC URL for confirmation, skipping");
        setSwapStatus("Transaction submitted successfully");
      }
      ...
      try {
        ...
        console.error("❌ [SWAP] Failed to save to database:", dbError);
  ...
  const saveSwapToDatabase = async (signature: string, quote: any) => {
    try {
      ...
      const result = await saveSwapTransaction(swapDetails, accessToken);
