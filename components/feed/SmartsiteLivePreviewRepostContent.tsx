...
const SmartsiteLivePreviewRepostContent = ({ feed }: any) => {
  ...
  return (
    <div className="flex gap-2 border border-gray-200 p-3 rounded-xl mt-2">
      <div className="w-6 h-6 bg-gray-400 border border-gray-300 rounded-full overflow-hidden flex items-center justify-center">
        {(() => {
          const profilePic =
                ...
                >
                  {feed.repostedPostDetails.content.title
                    ...
                    .map((line: string, index: number) => (
                      <p className="break-text text-start text-xs" key={index}>
                        {line}
                      </p>
                    ))}
                </button>
...
export default SmartsiteLivePreviewRepostContent;
