...
const SwapTransactionCard: React.FC<SwapTransactionCardProps> = ({ feed }) => {
  console.log("feed data hola for transaction", feed);
      ...
      >
        <div className="flex flex-col gap-3 border rounded-xl p-4 bg-white hover:bg-gray-50 transition-colors shadow-sm">
          ...
          <div className="flex items-center justify-between mt-2">
            {feed?.content && (
              <Link
                href={`/wallet?inputToken=${feed?.content?.inputToken?.symbol}&inputMint=${feed?.content?.inputToken?.mint}&inputChain=${feed?.content?.inputToken?.chain || feed?.content?.inputToken?.chainId}&inputImg=${encodeURIComponent(feed?.content?.inputToken?.tokenImg)}&inputDecimals=${feed?.content?.inputToken?.decimals}&outputToken=${feed?.content?.outputToken?.symbol}&outputMint=${feed?.content?.outputToken?.mint}&outputChain=${feed?.content?.outputToken?.chain || feed?.content?.outputToken?.chainId}&outputImg=${encodeURIComponent(feed?.content?.outputToken?.tokenImg)}&outputDecimals=${feed?.content?.outputToken?.decimals}&amount=${feed?.content?.inputToken?.amount}`}
