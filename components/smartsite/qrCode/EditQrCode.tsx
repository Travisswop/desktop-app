...
const EditQRCode = ({ qrCodeData, token }: any) => {
  ...
  const renderQRCode = (style: string, color: string, isPreview = false) => {
    const props = {
      height: isPreview ? 200 : 100,
      width: isPreview ? 200 : 100,
      color: isPreview ? color : style === qrPattern ? "white" : "black",
      className: isPreview ? "" : "-translate-x-[54px] -translate-y-[54px]",
