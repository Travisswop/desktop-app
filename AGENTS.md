...
### Personal-Branch Staging Source Of Truth

For production-level testing before merging to live, use the real GitHub
branches below as source of truth:

- Desktop app: `Travisswop/desktop-app`, branch `Codex`
- Main backend: `Travisswop/swop-app-backend`, branch `claude`
- Polymarket backend: `Travisswop/polymarket-backend`, branch `codex`

Do not push to or merge `main` unless Travis explicitly asks for the live
production step.

Local full-stack staging from `/Users/travis/Documents/Swop App`:

```bash
# Backend Eliza/Astro sidecar
cd /Users/travis/Documents/Swop\ App/swop-app-backend
ELIZA_SIDECAR_SECRET=dev-secret npm run eliza:start

# Main backend
cd /Users/travis/Documents/Swop\ App/swop-app-backend
PORT=4000 \
ELIZA_SIDECAR_SECRET=dev-secret \
AI_AGENT_ELIZA_SIDECAR_ENABLED=true \
ELIZA_SIDECAR_URL=http://127.0.0.1:4141 \
npm start

# Polymarket sidecar
cd /Users/travis/Documents/Swop\ App/polymarket-backend
PORT=5001 npm start

# Desktop
cd /Users/travis/Documents/Swop\ App/desktop-app
NEXT_PUBLIC_USE_AI_ONBOARDING=true npm run dev
```
...
- ElizaOS sidecar: http://127.0.0.1:4141
- Polymarket sidecar: http://localhost:5001

Relevant June 2, 2026 staging commits:

- Desktop `Codex`: `bdd3ff07 Add Hyperliquid perps feed cards`
- Backend `claude`: `92809a3 Add perps position feed endpoint`
- Backend `claude`: `a94360c Add Eliza sidecar routing for Astro`

Perps feed behavior to preserve:

- Feed cards render `postType: "perpsPosition"`.
- Backend endpoint is `POST /api/v2/feed/perps-position`.
- Position lifecycle supports `open`, `closed`, and `liquidated`.
- The card footer should say `Opened`, `Closed`, or `Liquidated` with the
  relevant timestamp.
- Return percent is lifecycle-aware: open cards use live/current mark price,
  closed cards use stored exit mark price, and liquidated cards use liquidation
  price when Hyperliquid provides it.
- Hyperliquid `userEvents`/`userFills` websocket subscriptions are used to mark
  liquidation fills as `liquidated`.

Known local noise:

- `package-lock.json` may show npm optional platform package pruning after local
  installs. Do not commit that churn unless lockfile normalization is explicitly
  requested.
- Polymarket may show `.DS_Store` deletions. Do not commit those as feature
  work.
