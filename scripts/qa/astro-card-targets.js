...
function listMatchingSwopChatTargets(targets, swopUrl) {
  return Array.isArray(targets) ? targets.filter((target) => isSwopChatTarget(target, swopUrl)) : [];
}

function findFreshSwopChatTarget(targets, swopUrl, staleTargetIds = []) {
  const staleIds = new Set(staleTargetIds);
  return listMatchingSwopChatTargets(targets, swopUrl).find((target) => !staleIds.has(target.id)) || null;
}
