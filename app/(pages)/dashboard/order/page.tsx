...
const detailHref = (row: OrderRow, tab: OrderTab) =>
  `/dashboard/order/${row.orderId}?tab=${tab.toLowerCase()}`;
...
export default function DashboardOrdersPage() {
  ...
  // initialLoading: true only until the first successful fetch — gates the
  // skeleton. isFetching: true during any in-flight request, including tab
  // switches; the screen stays mounted and surfaces a subtle progress bar.
  const [initialLoading, setInitialLoading] = useState(true);
  const [isFetching, setIsFetching] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(async (currentTab: OrderTab, token: string) => {
