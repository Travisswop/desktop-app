...
describe('astro-card target matching', () => {
  ...
  it('matches the requested chat route even when query strings or trailing slashes differ', () => {
    expect(matchesSwopChatUrl('http://localhost:3001/dashboard/chat?thread=astro', localhostChat)).toBe(true);
    expect(matchesSwopChatUrl('http://localhost:3001/dashboard/chat/', localhostChat)).toBe(true);
  });
  ...
  it('only treats same-origin chat tabs as reusable QA targets', () => {
    ...
    expect(listMatchingSwopChatTargets(targets, localhostChat).map((target) => target.id)).toEqual([
      'localhost-chat',
    ]);
  });
